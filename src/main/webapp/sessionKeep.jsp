...
<body>
	<%
		String session_id = session.getId(); // 세션 아이디 값
		long last_time = session.getLastAccessedTime(); // 세션 마지막 접근 시간
		long start_time = session.getCreationTime(); //  세션이 생성된 시간
		long keep_time = (last_time - start_time) / 1000;  // 분 단위로 출력 
		
		out.println("세션 아이디는 : " + session_id + "<br>");
